...
set "ROOT=%~dp0"
set "SCRIPT=%ROOT%start_widget_vt.py"
rem Dev-mode ROOT for this variant (see deskwidget_core/paths.py / appconfig.py)
rem is variants\vt, so that's where settings.json/the log file land too.
set "LOG=%ROOT%variants\vt\start_widget.log"
...
pushd "%ROOT%" >nul
start "VTDeskWidget" /b "%PYTHONW_EXE%" "%SCRIPT%"
