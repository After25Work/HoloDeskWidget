...
set "ROOT=%~dp0"
set "SCRIPT=%ROOT%start_widget_holo.py"
rem Dev-mode ROOT for this variant (see deskwidget_core/paths.py / appconfig.py)
rem is variants\holo, so that's where settings.json/the log file land too.
set "LOG=%ROOT%variants\holo\start_widget.log"
